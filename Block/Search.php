...
namespace Techvoot\StoreLocator\Block;
...
use Techvoot\Map\Api\MapInterface;
use Techvoot\Map\Api\MapProviderInterface;
use Techvoot\Map\Model\AddressFormatter;
use Techvoot\Retailer\Api\Data\RetailerExtensionInterface;
use Techvoot\Retailer\Api\Data\RetailerInterface;
use Techvoot\Retailer\Model\ResourceModel\Retailer\Collection;
use Techvoot\Retailer\Model\ResourceModel\Retailer\CollectionFactory as RetailerCollectionFactory;
use Techvoot\StoreLocator\Helper\Data;
...
class Search extends Template implements IdentityInterface
{
    ...
    public const CACHE_TAG = 'techvoot_store_locator_markers';
    ...
    public function getMarkers(): array
    {
        ...
        $cacheKey = sprintf('%s_%s_%s', 'techvoot_storelocator_search', $collection->getStoreId(), $todayDate);
        $markers = $this->cacheInterface->load($cacheKey);
        $attributes = $this->attributesToSelect;
        unset($attributes['name'], $attributes['url_key']);

        if (!$markers) {
            Profiler::start('TechvootStoreLocator:STORES');
            ...
            foreach ($collection as $retailer) {
                ...
                Profiler::start('TechvootStoreLocator:STORES_DATA');
                ...
                Profiler::stop('TechvootStoreLocator:STORES_DATA');
                ...
                Profiler::start('TechvootStoreLocator:STORES_SCHEDULE');
                ...
                Profiler::stop('TechvootStoreLocator:STORES_SCHEDULE');
                $marketDataObject = new DataObject($markerData);
                $this->_eventManager->dispatch('techvoot_store_locator_marker_data', ['data_object' => $marketDataObject]);
                $markers[] = $marketDataObject->getData();
            }
            Profiler::stop('TechvootStoreLocator:STORES');
